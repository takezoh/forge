...
class Database:
    ...
    async def upsert_issue(self, issue: Issue):
    ...
    async def get_issue(self, issue_id: str) -> Issue | None:
        cursor = await self._db.execute("SELECT * FROM issues WHERE id = ?", (issue_id,))
        row = await cursor.fetchone()
        if not row:
            return None
        return self._row_to_issue(row)

    async def get_issues_by_status(self, status: str) -> list[Issue]:
        cursor = await self._db.execute(
            "SELECT * FROM issues WHERE status = ? ORDER BY created_at", (status,))
        rows = await cursor.fetchall()
        return [self._row_to_issue(r) for r in rows]

    async def get_running_issues(self) -> list[Issue]:
        return await self.get_issues_by_status("running")

    async def update_status(self, issue_id: str, status: str, **kwargs):
        now = datetime.now(timezone.utc).isoformat()
        ...
        await self._db.execute(
            f"UPDATE issues SET {', '.join(sets)} WHERE id = ?", vals)
        await self._db.commit()

    async def log_event(self, issue_id: str, event_type: str, payload: dict | None = None):
        await self._db.execute(
    ...
    async def start_execution(self, issue_id: str, phase: str,
    ...
    async def finish_execution(self, exec_id: int, *, status: str = "done",
